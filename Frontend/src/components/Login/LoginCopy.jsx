...
const LoginCopy = () => {
  const [id, setId] = useState(''); // This will be used for user ID or admin ID
  ...
  const handleLogin = async (e) => {
    ...
    try {
      // Determine the correct login URL based on login type
      ...
      const response = await fetch(loginUrl, {
        ...
        body: JSON.stringify({
          user_id: !isAdminLogin ? id : undefined, // Only send user_id if not admin login
          admin_id: isAdminLogin ? id : undefined, // Only send admin_id if admin login
          password,
        }),
      });
      ...
      if (response.ok) {
        // Login success, display success message
        toast.success(data.message || 'Login successful!');

        // Save the token and other user data in local storage
        // Prepend "Bearer " to the token before saving it
